...
func (a *reportRepository) GetReportAccountBalance(UserID int64) (err error, results []models.AccountBalance) {
	sql := `select a."name" account_name, coalesce(je1.total,0) total_debit, coalesce(je2.total,0) total_kredit, coalesce((coalesce(je1.total,0) - coalesce(je2.total,0)),0) total from accounts a 
    left join (
